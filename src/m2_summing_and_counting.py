...
def run_test_sum_more_cosines():
    ...
    # Test 2:
    expected = 0.0208216577  # This is APPROXIMATELY the correct answer.
    answer = sum_more_cosines(-4, 1)
    print('Test 2 expected:', expected, '(approximately)')
    print('       actual:  ', answer)

    # Test 3:
    expected = 0.1020329  # This is APPROXIMATELY the correct answer.
    ...
    # Test 2:
    expected = -0.235818  # This is APPROXIMATELY the correct answer.
...
def sum_more_cosines(m, n):
    ...
    for k in range(n-m+1):  # Loop
        total = total + (math.cos(m+k))  # Accumulate INSIDE the loop.
...
def count_sines_vs_cosines(m):
    ...
    for k in range(m):  # Loop
        if math.sin(k + 1) > math.cos(k + 1):  # If the condition holds:
            count = count + 1  # Increment INSIDE the loop.
        if math.sin(-1*(k+1)) > math.cos(-1*(k+1)):
            count = count + 1
